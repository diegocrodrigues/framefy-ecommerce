                ...
                <?php
                    ...
                    if(isset($_SESSION['usuario']))
                    {
                        ...
                        if($linhas>0 )
                        {
                            ...
                            echo"<div class='estatistica'>";
                            $soma=0;
                            $receitaQuery=pg_query("SELECT * FROM public.compra;");
                            $receita=pg_fetch_all($receitaQuery);
                            
                            foreach($receita as $venda){
                                $soma+=$venda['total'];
                                
                                
                            }
                            echo "<div class='titulos'>Estatísticas:
                            <br><br></div>receita total: R$$soma <br>";
                            ...
                            $sexoQuery=pg_query("SELECT * FROM public.cadastro;");
                            $dadossexo=pg_fetch_all($sexoQuery);
                            
                            foreach($dadossexo as $user){
                                if($user['sexo']=='f') $qntmulher++;
                                if($user['sexo']=='m') $qnthomem++;
                                if($user['sexo']=='x') $outros++;
                                $total++;
                            }
                            $homem=number_format(($qnthomem/$total)*100,2);
                            $mulher=number_format(($qntmulher/$total)*100,2);
                            $outross=number_format(($outros/$total)*100,2);
                            
                            echo "
                            <br><div class='titulos'>Porcentagem de genêros cadastrados: <br></div>
                            <br><p>Homens:   $homem%</p>
                            <p>Mulheres: $mulher%</p>
                            <p>Outros:   $outross%</p>
                            ";
                            ...
                            $compraQuery=pg_query("SELECT * FROM public.compra;");
                            $compras=pg_fetch_all($compraQuery);
                            foreach($compras as $compra){
                                $prodQuery=pg_query("SELECT * FROM public.produto WHERE id_produto= $compra[id_produto];");
                                $prod=pg_fetch_array($prodQuery);
                                
                                if($prod['genero']=='Indie'){
                                    $qindie+=$compra['quantidade'];
                                }
                                
                                if($prod['genero']=='Pop'){
                                    $qpop+=$compra['quantidade'];
                                }
                                
                                if($prod['genero']=='Rock'){
                                    $qrock+=$compra['quantidade'];
                                }
                                
                                if($prod['genero']=='MPB'){
                                    $qmpb+=$compra['quantidade'];
                                }
                                
                                if($prod['genero']=='Eletrônica'){
                                    $qeletronica+=$compra['quantidade'];
                                }
                                
                                if($prod['genero']=='Rap'){
                                    $qrap+=$compra['quantidade'];
                                }
                            }
                            ...
                            echo "
                            <br><div class='titulos'>Compras por genêro musical: <br></div>
                            <br><p>Indie:   $qindie</p>
                            <p>Pop: $qpop</p>
                            <p>Rock:   $qrock</p>
                            <p>Rap:   $qrap</p>
                            <p>Eletrônica: $qeletronica</p>
                            <p>MPB:   $qmpb</p><br>
                            ";
